...
{
  den.aspects.home-assistant.nixos = { lib, config, ... }:
    let
      cfg = config.services.home-assistant-container;
    in
    {
      options.services.home-assistant-container.volumes = lib.mkOption {
        type = lib.types.listOf lib.types.str;
        description = "Volumes to mount (needs /config)";
      };

      options.services.home-assistant-container.addDbusVolume = lib.mkOption {
        type = lib.types.bool;
