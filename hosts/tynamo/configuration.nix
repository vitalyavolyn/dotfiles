    ...
    nixos = { pkgs, ... }:
      {
        ...
        # home LAN get routed into the tailscale0 tunnel and silently vanish
        # instead of going out over WiFi. Adding a more specific /24 route
        # for the home subnet into that same table fixes it via
        # longest-prefix-match. Only applies when wlp6s0 has a
